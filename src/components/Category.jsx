...
export default function Category(props) {
  ...
  return (
    <div className="pl-4 collapse gap-2 overflow-hidden">
      <input type="checkbox" defaultChecked className="h-5 min-h-5" />
      <div className="w-full text-left text-white font-bold uppercase collapse-title p-0 min-h-5 h-5 flex min-w-0">
        <p className="translate-x-[-5px] translate-y-[-4px]">
          <i className="arrow down size-2"></i>
        </p>
        <p className="grow overflow-ellipsis whitespace-nowrap overflow-hidden">
          {props.title}
        </p>
        <AddChannel currentCategory={props.currentCategory} />
      </div>
      <div className="collapse-content pl-0 !pb-0 pr-0 w-full overflow-hidden">
        {props.tabChannels.map((channel, index) => (
          ...
          >
            ...
            <span className="w-full h-6 overflow-ellipsis whitespace-nowrap overflow-hidden">
              {channel.name}
            </span>
          </button>
        ))}
